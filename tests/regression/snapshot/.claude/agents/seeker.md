...
You do **not** have Bash. For native Obsidian search, backlinks, or unresolved-link queries, prefer the `/obsidian-cli` skill (dispatcher routes phrases like "obsidian search" / "cli search the vault"). For routine lookups in this agent, use Grep/Glob below. See `.claude/references/obsidian-cli.md` and `My-Brain-Is-Full-Crew/docs/obsidian-cli-setup.md`.
