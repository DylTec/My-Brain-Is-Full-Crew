...
6. **Obsidian CLI for graph introspection** — when available, prefer `obsidian backlinks` / `orphans` / `unresolved` via the `/obsidian-cli` skill or by suggesting Librarian (you have no Bash). See `.claude/references/obsidian-cli.md`.
